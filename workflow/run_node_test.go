...
func TestRunNode_WithUseAsOutput_ChildOutputBecomesParentOutput(t *testing.T) {
	...
	n := NewDynamicNode[string, string]("orch",
	...
	// The delegated child's output-bearing event is suppressed so
	// the parent's terminal event is the only carrier of the value.
...
// Output is non-nil, preserving order. Used to assert that a
// WithUseAsOutput child does not duplicate the parent's terminal
// output event.
...
// parentTerminalOutput returns the Output of the last event stamped
// with parentPath — i.e. the terminal event yielded by
// dynamicNode.Run after the orchestrator body returns.
